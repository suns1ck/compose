...
def compose_collect(compose_dict):
    ...
    while additional_deployment != "N":
        ...
        if not os.path.exists(deployment_location):        
            ...
            except subprocess.CalledProcessError as failed_creation:
    ...
    for application_name, details in compose_dict.items():
        ...
        if urllib.parse.urlparse(compose_location).scheme:
            ...
            try:
                ...
                print(f"Failed to place {application_name} compose file: {fail}")
                continue
            ...
            except subprocess.CalledProcessError as failed_copy:
                print(f"Failed to copy {application_name} compose file to {deployment_directory}")
                continue

def compose_deploy(compose_dict):

    # This function will iterate through the deployment location(s) and deploy all specified containers/applications.
    ...
    for application_name, details in compose_dict.items():
        ...
        except subprocess.CalledProcessError as failed_deployment:
            print(f"Your container failed to deploy: {failed_deployment}")
